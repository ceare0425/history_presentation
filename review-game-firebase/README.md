...
관리자 화면 "⑥ 학생 명단 관리"에서 반별로 학생 이름을 한 줄에 하나씩 등록해두면, `solo.html`(혼자 연습)
시작 시 그 반은 **명단에 있는 이름만** 입력할 수 있습니다. 오타나 IME 조합 중 깨진 이름(예: 자모가
분리된 이름)으로 기록이 남는 것을 막기 위한 기능입니다. 한국사는 1학년, 세계사는 2학년이 쓰고 반
번호(1~10반)가 두 학년에서 겹칠 수 있으므로, 명단은 `solo_log`와 마찬가지로 **방마다**
(`rooms/world/roster/<반>`, `rooms/korea/roster/<반>`) 따로 저장됩니다 — 한쪽에서 등록해도 다른
과목/학년에는 영향을 주지 않습니다. 화면에도 몇 학년 명단인지 표시됩니다.
**명단을 한 명도 등록하지 않은 반은 제한 없이** 누구나 이름을 입력해 연습할 수 있습니다.
...
한 줄씩 입력하는 대신, 학교에 이미 있는 **전체 반 명렬표 엑셀 파일을 그대로 업로드**할 수도 있습니다:

- 시트가 여러 개면 시트 이름에 들어있는 숫자(예: "1반", "3-2")를 반 번호로 인식하고, 시트 안의
  "이름"(또는 "성명") 열을 읽습니다.
- 시트가 하나뿐이면 그 안에 "반"(또는 "학급") 열과 "이름"(또는 "성명") 열이 함께 있어야 합니다.
- 업로드 전 몇 개 반에서 몇 명을 찾았는지 확인 창이 뜨고, 확인을 누르면 그 반들의 기존 명단을 파일
  내용으로 교체합니다(업로드에 없는 다른 반은 그대로 유지).
- 파일 형식이 애매하면(예: 시트 이름에 학년과 반이 같이 들어가 숫자가 여러 개인 경우) 확인 창의
  반별 인원 수를 보고 예상과 다르면 취소한 뒤 "명단 양식 다운로드"에 맞춰 다시 정리하세요.
