...
학생이 연습 시작 화면에서 **반(1~10반)과 이름**을 넣고 연습을 끝내면(제한 시간 종료 시), 그 세션 요약이
`rooms/<room>/solo_log`에 기록됩니다: 반 · 이름 · 시각 · 범위 · 최종 층수 · 정답/오답 수.
관리자 화면 맨 아래 **"⑤ 혼자 연습 현황"** 카드에서:

- 총 연습 횟수 / 오늘 / 최근 7일 / 연습한 학생 수
- **학생별 표**: 반, 이름, 횟수, 평균 층수, 최고 층수, 최근 연습 시각 (반 순 정렬)
- **최근 세션 목록** (최대 40개): 개별 연습 기록 (반 포함)
- **"기록 비우기"** 버튼으로 solo_log 전체 삭제 (학생 점수·참여 목록과 무관)

**반과 이름은 둘 다 필수**입니다 — 하나라도 비면 "연습 시작"이 눌리지 않습니다. 반은 한 번 고르면
localStorage에 저장돼 다음에도 유지되고(세계사/한국사 공용), 이름은 같은 기기로 `play.html`에 참여한 적이
