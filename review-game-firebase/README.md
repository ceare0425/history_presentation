...
**쪽지** 버튼을 누르면 그 학생과 대화창이 열립니다. 학생 쪽도 관리자처럼 화면에 쪽지가 바로 튀어나오지
않고 상단의 **"💬쪽지" 버튼에 안 읽은 개수 배지**만 뜹니다 — 학생이 그 버튼을 눌러야 내용이 보이고,
같은 버튼으로 먼저 선생님께 말을 걸 수도 있어 관리자 ↔ 학생 양방향 대화가 가능합니다. 학생이 쪽지를
못 보고 연습을 끝내거나 시간이 다 되면, **연습 종료 화면 위에 팝업**으로 놓친 쪽지를 띄워 그 자리에서
바로 답장하거나 확인 처리할 수 있습니다. 학생이 연습 중이 아니거나 창을 닫으면(또는 네트워크가
끊기면) 온라인 목록에서 자동으로 사라지지만, 나눈 대화 내용은 계속 남아 있어 다음에 다시 쪽지
버튼을 누르면 이어서 볼 수 있습니다. 카카오톡처럼 **상대방이 확인하면 "1"이 사라지는 읽음 표시**도
있습니다 — 관리자 대화창에서는 선생님이 보낸 말풍선에 학생이 아직 확인 안 했으면 "1"이 붙어 있다가,
학생이 쪽지 버튼을 눌러 열거나(놓쳤을 땐 종료 팝업에서 확인/답장하거나) 하면 사라집니다. 반대로
학생이 보낸 답장은 선생님이 그 학생과의 대화창을 열어 실제로 볼 때 읽음 처리되어, 학생 쪽 배너에
"선생님이 읽었어요"로 표시됩니다.
