...
조합 중 깨진 이름(예: 자모가 분리된 이름)으로 기록이 남는 것을 막기 위한 기능입니다. 반 번호(1~10)가
학년마다 있을 수 있으므로(1학년 3반과 2학년 3반은 다른 반) 명단은 `solo_log`와 마찬가지로 **방마다**,
그 안에서도 **"학년-반" 합친 키**로 저장됩니다(`rooms/world/roster/<학년>-<반>`,
`rooms/korea/roster/<학년>-<반>`). **명단을 한 명도 등록하지 않은 학년·반은 제한 없이** 누구나
이름을 입력해 연습할 수 있습니다.
