...
### 학생 명단 관리 (관리자 화면 "⑥", 혼자 연습 이름 제한)

관리자 화면 "⑥ 학생 명단 관리"에서 **학년·반**을 고르고 학생 이름을 한 줄에 하나씩 등록해두면,
`solo.html`(혼자 연습) 시작 시 그 학년·반은 **명단에 있는 이름만** 입력할 수 있습니다. 오타나 IME
조합 중 깨진 이름(예: 자모가 분리된 이름)으로 기록이 남는 것을 막기 위한 기능입니다. "학년-반"은
...
### 학생용 시작 화면 (`student.html`)과 이름 자동 채움
...
저장되고, 이 값을 `solo.html`과 `play.html`이 그대로 읽어 이름칸을 미리 채웁니다:

- `solo.html`은 학년·반 버튼이 이미 선택된 채로, 이름칸도 채워진 채로 열립니다(기존 동작과 동일).
- `play.html`(수업 참여)의 이름칸도 "1학년 3반 홍길동" 형태로 미리 채워집니다. **다만 자동으로
  참여시키지는 않고**, 학생이 이름을 확인/수정한 뒤 직접 "참여하기"를 눌러야 들어가집니다.

> ⚠️ `play.html` 이름칸을 미리 채우는 건 **기기를 한 학생이 계속 쓴다는 전제**입니다. 여러 학생이
> 기기(예: 학교 공용 태블릿)를 돌려 쓰는 경우, 이전 학생 이름이 채워진 채로 남아있을 수 있으니
> 참여 전에 이름을 꼭 확인하도록 안내하세요.

`student.html`을 거치지 않고 `ceare0425.github.io/w`, `/k`처럼 과목별 주소로 바로 들어가도
지금까지 저장된 값이 있으면 똑같이 미리 채워지고, 없으면(첫 방문) 그냥 빈 채로 시작합니다 —
기존 북마크/QR코드도 그대로 동작합니다.
...
review-game-firebase/
  index.html              메인 페이지 (교사용 - 관리자 화면 링크 + 학생 화면(student.html) 바로가기, 대형 화면은 admin.html의 버튼으로 엶)
